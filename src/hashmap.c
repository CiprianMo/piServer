...
/* Simple Bob Jenkins's hash algo*/
...
static inline DArray *Hashmap_find_bucket(Hashmap *map, void *key, int create, uint32_t *hash_out)
{
    ...
    check(bucket_n >= 0,"Ivalide bucket found : %d", bucket_n);
...
void *Hashmap_get(Hashmap *map, void *key)
{
        ...
        HashmapNode *node = DArray_get(bucket,i);
        check(node != NULL,"Failed to get the node from the bucket when it should exit");
